...
1. 我的理解是，一个域下的 js 脚本在未经允许的情况下，不能够访问另一个域的内容。这里的同源的指的是两个域的协议、域名、端口号必须相同，否则则不属于同一个域；目的主要是为了保证用户的信息安全。
2. 同源策略主要限制了 3 个方面：
...
## HTTP 请求头中的 Content-Type 有哪些

1. application/json
2. application/octet-stream：二进制流，文件下载一般是这个
3. image/png：image 开头的图片，此外还有 gif、jpeg
4. multipart/form-data：form 表单提交
